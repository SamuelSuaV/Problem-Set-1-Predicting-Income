...
- **`latex/data_description.tex`** — the writeup: pulls every table/figure from `output/` via `\input{../output/tables/*.tex}` / `\includegraphics{../output/figures/*.png}`, so it stays in sync automatically when `02`/`03` are rerun — no content is duplicated into the `.tex` file itself. Compile with `pdflatex` from inside `latex/` (twice, for the TOC/cross-references). `latex/missing_income.tex` is a standalone note on the `y_total_m` missingness; `latex/section3_best_model_slide.tex` is an `\input` fragment for the prediction deck.
- **`00_master_file.r`** — working orchestrator: sources `script/01`–`08` in order in isolated environments, accepts step-prefix args (`Rscript script/00_master_file.r 04 05`) to run a subset, and stops at the first failing step. Step 01 (scraping) **always runs** and overwrites `data/geih_scrap.rds` on every full-pipeline run.
...
- Run the whole pipeline: `Rscript script/00_master_file.r` (or `Rscript script/00_master_file.r 04 05` for a subset by step prefix). Step 01 always re-scrapes.
...
- Compile the writeup: `cd latex && pdflatex -interaction=nonstopmode data_description.tex && pdflatex -interaction=nonstopmode data_description.tex` (two passes; run after `02`/`03` so the `.tex`/figure inputs are current). Slide decks compile from `latex/presentation/`. LaTeX/beamer build artifacts (`*.aux`/`*.log`/`*.out`/`*.toc`/`*.nav`/`*.snm`/`*.synctex.gz`/…) and `*.pdf` are all `.gitignore`d.
...
- `latex/presentation/` holds slide-deck sources, but `.gitignore` excludes `*.pdf`/`*.html` repo-wide, so rendered decks aren't committed here — they're submitted separately on Bloque Neón as `age_equipo_XX.pdf`, `gap_equipo_XX.pdf`, `pred_equipo_XX.pdf`.
