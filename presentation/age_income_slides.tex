...
% Palette matches the age-income profile figure (04_age_labor_income.r).
\definecolor{uandespurple}{HTML}{6C0A8A}
\definecolor{uandesblue}{HTML}{4DAAD5}
...
\input{../output/tables/age_income_stats.tex}

\newcommand{\teamnumber}{XX}

\title[Perfil edad--ingreso]{Perfil edad--ingreso laboral en Bogotá}
\subtitle{Problem Set 1 \textbullet\ Sección 1 \textbullet\ Equipo \teamnumber}
\author[Equipo \teamnumber]{María José Pérez \and Juan Manuel Lozano \and Samuel Suárez Valle}
  ...
  \begin{block}{El ingreso laboral crece con la edad, alcanza un máximo y luego decae}
    \begin{itemize}
      ...
      \item \textbf{Perfil condicional} (añade horas trabajadas y tipo de empleo):
        ...
        \ $R^2 = \AgeRsqCond$.
      \item El máximo cae \emph{dentro} del rango de edad observado y es coherente con la
        teoría del capital humano: la inversión temprana se amortiza y después domina
        la depreciación.
    \end{itemize}
  \end{block}
  \vfill
...
\begin{frame}{Pregunta y marco teórico}
  ...
  \vspace{0.6em}
  \begin{itemize}
    \item Teoría del capital humano (Mincer): los retornos a la experiencia son
      decrecientes $\Rightarrow$ el perfil edad--ingreso es cóncavo.
    \item Una especificación cuadrática en la edad captura esa concavidad y entrega
      una \emph{edad pico} interpretable, $\text{edad}^\ast = -\beta_2 / (2\beta_3)$.
    \item Uso tributario: el perfil fija un ingreso esperado por edad; desviaciones
      sistemáticas por debajo de ese perfil son una señal de posible sub-reporte.
  \end{itemize}
\end{frame}
...
\begin{frame}{Datos y decisiones empíricas}
  \begin{itemize}
    ...
    \item \textbf{Ingresos no positivos:} se excluyen (el logaritmo no está definido).
      La no respuesta en \texttt{y\_total\_m} se concentra en trabajadores familiares
      sin remuneración (\texttt{relab} 6--7), fuera del margen de interés.
    \item \textbf{Ponderación:} todas las regresiones usan \texttt{fex\_c}
      (factor de expansión). El perfil describe a los trabajadores de Bogotá,
      no a los \AgeNobs\ respondientes observados.
  \end{itemize}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{Evidencia descriptiva: los ingresos medios ya dibujan una joroba}
  \centering
  ...
  edad. El grupo 39--50 tiene la media más alta; el grupo 51--94 retrocede.}
\end{frame}
...
\begin{frame}{Especificaciones}
  ...
  \textbf{Condicional} (añade horas y tipo de empleo, sin otros controles, según el enunciado)
  \[
...
\begin{frame}{Comparación de especificaciones}
  \centering
  {\small \input{../output/tables/age_income_slide_table.tex}}
...
\begin{frame}{Perfiles edad--ingreso estimados}
  \centering
  \includegraphics[height=0.82\textheight]{../output/figures/age_income_profile.png}
\end{frame}
...
\begin{frame}{Edad pico e intervalo de confianza}
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      \textcolor{uandespurple}{\textbf{Incondicional}}
      \begin{itemize}
      ...
      \end{itemize}
      \vspace{0.6em}
      \textcolor{uandesblue}{\textbf{Condicional}}
      \begin{itemize}
    ...
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item Ambos intervalos son estrechos ($\approx 1$ año de ancho) y quedan
          cómodamente dentro del rango de edad observado.
        \item Al condicionar por horas y tipo de empleo, la edad pico se
          ...
          se aplana.
        \item Lectura: parte del crecimiento temprano del ingreso no es la edad en
          sí, sino más horas trabajadas y el paso hacia mejores modalidades de empleo.
          Al mantenerlas fijas, el retorno puro a la edad es más plano y tardío.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusiones}
  \begin{itemize}
    ...
    \item Para la autoridad tributaria: la edad da una referencia de ingreso esperado
      por ciclo de vida, pero es un predictor débil por sí sola. Debe combinarse con
